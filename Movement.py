...
def movement(N, n, x_min = -20, x_max = 20, y_min = -20, y_max = 20, vx_max = 5, vy_max = 5): #master function for driving the simulation. N is the number of collision to simulate, n is the number of particles
    ...
    fig1, axs = plt.subplots(11, 1, constrained_layout = True)
    for _ in range(N):
        ...
        if _ % (N/10) == 0:
            ...
            for i in range(50):
                ...
                bar.append(0)
            for p in particles.array:
                bar[int((p.vx**2 + p.vy**2)**0.5 * 50 / 10)] += 1 / n
            axs[int(_/(N/10))].bar(bar_pos, bar, width= 10 / 50, align='edge')
            axs[int(_/(N/10))].set_title('maxwell - boltzmann after {} collisions'.format(_))
            k = 10 ** (-23)
            T = 0.4
            m = 0.02
#            for x in range(0, 1000):
#                plt.plot(x / 100, 4 * math.pi * ((m * k) / (2 * math.pi * 1.38064852 * k * T))**1.5 * (x / 100)**2 * math.e ** ((- m * k * (x/100)**2) / (2 * 1.38064852 * k * T)), 'r,')
#            print(sum(bar))
            plt.show()
